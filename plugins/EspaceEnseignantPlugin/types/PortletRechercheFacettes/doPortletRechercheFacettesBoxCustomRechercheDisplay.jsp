...
<% 
    ...
    // Override Digiwin
    Object isSmallSearchObj = request.getAttribute("isSmallSearch");
    Boolean isSmallSearch = Util.notEmpty(isSmallSearchObj) ? (Boolean)isSmallSearchObj : false;
    isInRechercheFacette = isInRechercheFacette && !isSmallSearch;
    // END Override
    ...
    // Override Digiwin add !isSmallSearch
    Boolean showFiltres = !isSmallSearch && (isInRechercheFacette && Util.notEmpty(obj.getFacettesSecondaires()) || hasFonctionsAdditionnelles);
...
<div class="ds44-facette">
    <div class="ds44-facette-body">
        ...
        <%-- Override Digiwin add isSmallSearch --%>
        <jalios:if predicate='<%= !isSmallSearch && (obj.getAfficherResultatDansLannuaire() || (!isInRechercheFacette && Util.notEmpty(obj.getTitre(userLang)))) %>'>
        ...
        <form role="search" method='<%= channel.getBooleanProperty("jcmsplugin.socle.url-rewriting", false) ? "POST" : "GET" %>' data-seo-url='<%= channel.getProperty("jcmsplugin.socle.url-rewriting")%>' data-search-url="plugins/SoclePlugin/jsp/facettes/displayParameters.jsp" data-is-ajax='<%= isInRechercheFacette ? "true" : "false" %>' data-auto-load='<%= isInRechercheFacette ? "true" : "false" %>' action='<%= isInRechercheFacette ? "plugins/SoclePlugin/jsp/facettes/displayResultDecodeParams.jsp" : seoUrl + "?boxId=" + obj.getId() %>'>
            ...
            <%-- Override Digiwin --%>
            <jalios:if predicate="<%= !isInRechercheFacette && isSmallSearch %>">
                <%
                String inputName = (String) request.getAttribute("rechercheFilter");
                Category catFilter = (Category)request.getAttribute("rechercheFilterCatVal");
                %>
                
                <jalios:if predicate="<%= Util.notEmpty(inputName) && Util.notEmpty(catFilter) %>">
                    <div class="hidden">
                        <div id="<%= inputName %>" data-name="<%= inputName %>" class="ds44-form__checkbox_container ds44-form__container"  data-required="true"
                                title="<%= catFilter.getName() %>">
                            <div class="ds44-form__container ds44-checkBox-radio_list ">
                                <input type="checkbox" id="<%= inputName %>-val" name="<%= inputName %>-val" value="<%= catFilter.getId() %>" 
                                        class="ds44-checkbox" required checked="checked" />
                                <label for="<%= inputName %>-val" class="ds44-boxLabel" id="<%= inputName %>-lbl"><%= catFilter.getName() %></label>
                            </div>
                        </div>
                    </div>
                </jalios:if>
